---
name: spec-first
description: 管理项目模块的完整开发周期：需求分析→更新技术方案→spec确认→开发→验收→收尾。适用于功能新增、Bug修复、需求变更、接口调整、重构、字段增删等涉及代码变更的任务；项目下有或无 spec 文档均可触发，无需用户明确指定流程。
---
...
- **存在但缺少上述章节** → 立即分析项目文件推断缺失内容并补全写入 `CLAUDE.md`（参照 `assets/claude-md-snippet.md` 对应章节格式；无法推断的字段保留占位符），继续 ②
- **不存在** → 读取 [assets/claude-md-snippet.md](assets/claude-md-snippet.md)，在项目根目录创建 `CLAUDE.md` 并写入完整片段内容（占位符保留，后续各阶段执行时再实际填写），继续 ②
